...
public class LightTest {
	...
	public void shadowTest() {
		...
		scene.setGeometries(geometries);
		Sphere sphere = new Sphere(new Material(0.5, 0.5, 300), new Color(0, 0, 100), 50, new Point3D(0, 0, 100));
		Triangle triangle = new Triangle(new Material(0.5, 0.5, 300), new Color(0, 0, 100), new Point3D(-30, 25, 40),
		...
		triangle = new Triangle(new Material(0.5, 0.5, 300), new Color(0, 0, 100), new Point3D(-30, 20, 40),
		...
		triangle = new Triangle(new Material(0.5, 0.5, 300), new Color(0, 0, 100), new Point3D(-20, 20, 40),
		...
		Triangle triangle1 = new Triangle(new Material(0.5, 0.5, 300), new Color(0, 0, 100), new Point3D(-30, 20, 40),
				new Point3D(-30, 10, 40), new Point3D(-20, 20, 40));
	...
	public void transparencyTest() {
		...
		Geometries geometries = new Geometries();
		scene.setGeometries(geometries);
		Sphere sphere1 = new Sphere(new Material(0.5, 0.5, 300, 0, 0.3), new Color(0, 0, 100), 30,
				new Point3D(0, 0, 60));
	...
	@Test
	public void DepthOfFieldTest1() {
		Scene scene = new Scene("Depth of field tests1");
		scene.setCamera(new Camera(new Point3D(0, 0, 0), new Vector(0, -1, 0), new Vector(0, 0, -1)));
		scene.setScreenDistance(300);
		scene.setAmbientLight(new AmbientLight(new Color(20, 0, 0), 1));
		scene.setBackground(Color.BLACK);
		Geometries geometries = new Geometries();
		scene.setGeometries(geometries);

		geometries.add(
				new Plane(new Material(0.5, 0.5, 500, 0, 1), new Color(50, 50, 50), new Point3D(0, 50, 60),
						new Vector(0, 1, 0)),
				new Sphere(new Material(0.5, 0.5, 1200, 0, 0), new Color(0, 30, 0), //
						20, new Point3D(60, 0, -140)), // sphere 1
				new Sphere(new Material(0.5, 0.5, 1200, 0, 0), new Color(0, 0, 20), //
						20, new Point3D(0, 0, -160)), // sphere 2
				new Sphere(new Material(0.5, 0.5, 1200, 0, 0), new Color(30, 0, 0), //
						20, new Point3D(-75, 0, -200)) // sphere 3
		);

		ImageWriter imageWriter = new ImageWriter("fdv1", 500, 500, 600, 600);
		List<LightSource> lights = new ArrayList<LightSource>();
		PointLight light1 = new PointLight(new Color(400, 300, 300), new Point3D(-50, 50, 0), 1, 0.00005, 0.0000005);
		lights.add(light1);
		DirectionalLight directionalLight = new DirectionalLight(new Color(300, 150, 150), new Vector(1, -1, 2));
		lights.add(directionalLight);
		scene.setLights(lights);
		Render render = new Render(imageWriter, scene);
		render.renderImageDOF(new Sphere(100, new Point3D(170, 0, -350)), 375);
		render.writeToImage();
	}
	@Test
	public void dof() {
		Scene scene = new Scene("Test dof");
		scene.setCamera(new Camera(new Point3D(0, 0, 0), new Vector(0, -1, 0), new Vector(0, 0, 1)));
		...
		geometries.add(new Triangle(new Color(java.awt.Color.red),new Point3D(100, 0, 149), new Point3D(0, 100, 149), new Point3D(100, 100, 149)));

		geometries.add(new Triangle(new Color(java.awt.Color.yellow),new Point3D(100, 0, 149), new Point3D(0, -100, 149), new Point3D(100, -100, 149)));

		geometries.add(new Triangle(new Color(java.awt.Color.blue),new Point3D(-100, 0, 149), new Point3D(0, 100, 149), new Point3D(-100, 100, 149)));

		geometries
				.add(new Triangle(new Color(java.awt.Color.green),new Point3D(-100, 0, 149), new Point3D(0, -100, 149), new Point3D(-100, -100, 149)));

		ImageWriter imageWriter = new ImageWriter("DOF", 500, 500, 500, 500);
		Render render = new Render(imageWriter, scene);

		render.renderImageDOF(new Sphere(50, new Point3D(0, 0, 50)), 100);
		render.writeToImage();
	}
}